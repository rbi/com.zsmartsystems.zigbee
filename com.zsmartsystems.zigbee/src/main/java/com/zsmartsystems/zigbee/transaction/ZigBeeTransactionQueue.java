...
public class ZigBeeTransactionQueue {
    ...
    /**
     * Adds a {@link ZigBeeTransaction} to the queue, returning a {@link CommandResult} Future that will be fullfilled
     * once the transaction completes.
     *
     * @param command {@link ZigBeeTransaction}
    ...
    protected Future<CommandResult> addToQueue(ZigBeeTransaction transaction) {
        if (profile.isDuplicateRemoval()) {
            // ZigBeeTransaction duplicate = queue..remove(transaction);
            // logger.debug("{}: Removed duplicate transaction: ", queueName, duplicate);

            // We found a duplicate command.
            // Remove the original and queue the new command below

            // Return the original Future, so when we complete the transaction, we complete all listeners
            // transaction.setFuture(duplicate.getFuture());
        }
